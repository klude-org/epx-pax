...
class epx__250328_01_db__pax__klude_org__github implements \ArrayAccess {
    ...
    private function __construct(int $index = 0){
        $this->hostname = $_ENV[static::class]['HOSTNAME'] ?? $_ENV['DB_HOSTNAME'] ?? 'localhost';
        $this->database = $_ENV[static::class]['DATABASE'] ?? $_ENV['DB_DATABASE'] ?? '';
        $this->char_set = $_ENV[static::class]['CHAR_SET'] ?? $_ENV['DB_CHAR_SET'] ?? 'utf8mb4';
    ...
    public function query(string|callable $query = null,...$args){
        if(\is_string($query)){
            try{
                ...
                throw new \epx__250328_01_db__pax__klude_org__github\fault\pdo_query('Encountered an exception on query', 0, $ex, [
                    'previous' => 1,
                    'sql' => $query
                ]);
            }
        } else if(\is_callable($query) || \is_null($query)){
            $q = \epx__250328_01_db__pax__klude_org__github\query::_($this);
            if($query){
                ($query)($q, $this);
            }
            return $q;
    ...
    public function exec(string $query): int|false {
        try{
            ...
            throw new \epx__250328_01_db__pax__klude_org__github\fault\pdo_query('Encountered an exception on exec', 0, $ex, [
                'previous' => 1,
                'sql' => $query
            ]);
        }
    ...
    public function offsetGet($n):mixed {
        return $this->i__tables[$n]['node'] ?? (
            $this->i__tables[$n]['node'] = new \epx__250328_01_db__pax__klude_org__github\table($this, $n)
        );
    }
    ...
    public function table__fsd($tblp){
        return $this->i__tables[$tblp]['fsd'] ?? (
            $this->i__tables[$tblp]['fsd'] = \epx__250328_01_db__pax__klude_org__github\fsd::new_($this, "{$this->dir}/{$tblp}")
        );
    }
    ...
    public function get($expr){
        if(\is_string($expr)){
            return \epx__250328_01_db__pax__klude_org__github\get::new_($this, $expr);
        } else if(\is_callable($expr)){
            $query = \epx__250328_01_db__pax__klude_org__github\query::new_($this);
            if($expr){
                ($expr)($query, $this);
            }
            return \epx__250328_01_db__pax__klude_org__github\get::new_($this, $query->sql());
        } else if($expr instanceof \epx__250328_01_db__pax__klude_org__github\query){
            return \epx__250328_01_db__pax__klude_org__github\get::new_($this, $expr->sql());
        } else {
    ...
    public function query_builder(){
        return \epx__250328_01_db__pax__klude_org__github\query::new_($this);
    }
