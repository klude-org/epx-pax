...
<head>
    ...
    <?php if($v = $this->env->vars['html']['tab_title'] ?? null): ?><title><?=$v?></title><?php endif ?>
    <?php if($v = $this->env->vars['html']['tab_icon'] ?? null): ?><link rel="icon" href="<?=$v?>"><?php endif ?>
    <?php if($v = $this->env->vars['html']['base'] ?? null): ?><base href="<?=$v?>"><?php endif ?>
    <?php if($v = $this->env->vars['html']['canonical'] ?? null): ?><link rel="canonical" href="<?=$v?>"><?php endif ?>
    <?php foreach($this->env->vars['html']['meta'] ?? [] as $k => $v): ?><meta name="<?=$k?>" content="<?=$v?>"><?=PHP_EOL; endforeach ?>
...
<body>
    ...
    <div class="xui-container">
        <div class="xui-sidebar p-1" id="leftSidebar">
            <div class="d-flex justify-content-between align-items-center mb-1">
                <a href="<?=$this->env->site_url ?? ''?>" class="xui-nav-header d-flex align-items-center mb-md-0 me-md-auto text-decoration-none">
    ...
    <script>
        ...
        const CTLR_URL = '<?=$this->env->ctlr_url ?? ''?>'
        const BASE_URL = '<?=$this->env->base_url ?? ''?>'
        const SITE_URL = '<?=$this->env->site_url ?? ''?>';
        const ROOT_URL = '<?=$this->env->root_url ?? ''?>';
